...
## Физическая модель
<img width="1249" height="979" alt="физ" src="https://github.com/user-attachments/assets/a22eb3ff-84fb-4cc5-8459-f59bbd7dfb65" />
...
| passport_id | UUID | FK | Паспорт. Обязательным должно быть хотя бы одно из двух полей: passport_id, certificate_id |
| certificate_id | UUID | FK | Свидетельство о рождении. Обязательным должно быть хотя бы одно из двух полей: passport_id, certificate_id |
| inn | INTEGER | NOT NULL | ИНН |
| date_create | TIMESTAMP | NOT NULL, DEFAULT NOW() | Дата создания |
| date_update | TIMESTAMP | CHECK (date_update >= date_create) | Дата последнего изменения |
...
## Диаграмма классов
<img width="523" height="692" alt="класс" src="https://github.com/user-attachments/assets/86cf60d1-50ee-4db5-b0d1-2db797ddb07d" />
...
```json
{
  ...
  "properties": {
    ...
    "recipient": {
      "type": "object",
      "additionalProperties": false,
      "required": ["type", "surname", "name", "birthDate", "inn"],
      "properties": {
        "type": {
          "type": "string",
