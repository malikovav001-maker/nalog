...
if (Данные отобразились?) then (да)
    ...
    if (Отобразилась категория "Обучение"?) then (да)
        ...
        if (Компания является партнером?) then (Да)
            ...
            repeat while (Данные сохранились?) is (нет)
            ...
            repeat
                :Отправить заявление\nв компанию-партнер;
            repeat while (Отправка успешная?)
            ...
            if (Статус подготовки = "Сформировано"?) then (да)
                    ...
                    |Интеграционный шлюз|
                    :Обработать подпроцесс "Опрос статуса ФНС";
    ...
    |Интерфейс|
    :Отобразить подсказку:\n"Нет операций для получения налогового вычета";
endif
...
@enduml
```
## Подпроцесс "Опрос статуса ФНС"

```plantuml
@startuml диаграмма деятельности
title Подпроцесс "Опрос статуса ФНС"
start
          repeat
          :Периодический опрос статуса ФНС (каждые 24 часа);
          repeat while (Новый статус?)
